    ...
    this.courseService.getCourses(courseFilters, sortOptions, paginationOptions)
      .subscribe({
        next: (response: PaginatedResult<DomainCourse>) => {
          // Convert domain courses to UI courses
          const mapped: ExtendedCourse[] = response.items.map(c => ({
            id: c.id as unknown as string,
            title: c.title,
            description: c.description,
            shortDescription: c.shortDescription,
            level: (c.specifications.level as any) || 'beginner',
            duration: c.getFormattedDuration(),
            students: c.metadata.studentsCount,
            rating: c.getRating(),
            reviews: c.metadata.reviewsCount,
            price: c.specifications.price,
            originalPrice: undefined,
            instructor: {
              id: c.instructorId as unknown as string,
              name: 'Giảng viên',
              title: 'Chuyên gia',
              ...
              experience: 0,
              rating: c.getRating(),
              studentsCount: c.metadata.studentsCount
            },
            thumbnail: c.thumbnail || 'assets/images/courses/placeholder.png',
            category: c.category,
            tags: c.tags,
            skills: c.skills,
            prerequisites: [],
            certificate: { type: 'Completion', description: '' },
            curriculum: { modules: 0, lessons: 0, duration: c.getFormattedDuration() },
            isPopular: c.metadata.isPopular,
            isNew: c.metadata.isNew,
            isFree: c.specifications.price === 0,
            studentsCount: c.metadata.studentsCount,
            lessonsCount: 0,
